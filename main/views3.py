...
def kassa(request):
    ...
    kirim_qs = Kirim.objects.filter(is_approved=True, kassa__kassa__is_main=True).filter(date_filters & base_filters)
    chiqim_qs = Chiqim.objects.filter(is_approved=True, kassa__kassa__is_main=True).filter(date_filters & base_filters)
    ...
    # Optimize kassa data
    kassa_data = []
    active_kassa = KassaMerge.objects.filter(is_active=True, kassa__is_main=True)
    
    for kassa_obj in KassaNew.objects.filter(is_main=True):
        valyuta_data = []
...
def shop_nakladnoy(request, order_id):
    try:
        ...
        shop = Shop.objects.select_related(
            'debtor', 'saler', 'valyuta', 'filial'
        ).prefetch_related(
            Prefetch(
                'cart',
                queryset=Cart.objects.select_related('product', 'product__deliver1'),
                to_attr='carts'
            ),
            Prefetch(
                'payhistory_set',
            ...
            Prefetch(
                'cart__returns',
                queryset=ReturnProduct.objects.select_related('by_user', 'by_user__user'),
                to_attr='returned_items'
            )
        ).get(id=order_id)
        ...
        for cart in shop.carts:
            ...
            data['items'].append({
                ...
                'nds_price': nds_price,
                'nds_total': nds_total,
                'deliver': cart.product.deliver1.name if cart.product.deliver1 else '',
        ...
        for payment in shop.payment_history:
            payment_type = "Qarz" if payment.is_debt else "To'lov"
            payment_data = {
                'summa': payment.summa,
                'valyuta': payment.valyuta.name if payment.valyuta else '',
                'kassa': payment.kassa.kassa.name if payment.kassa and payment.kassa.kassa else 'Noma\'lum',
            ...
            if payment.is_debt:
                debt_payments += payment.summa
            else:
                if payment.valyuta.is_dollar:
                    total_paid += payment.summa * shop.kurs
                else:
                    total_paid += payment.summa

        
        total_returned = 0
        return_payment = 0

        for chiqim in shop.chiqims.all():
            return_pay_data = {
                'summa': chiqim.summa,
                'valyuta': chiqim.valyuta.name if chiqim.valyuta else '',
                'kassa': chiqim.kassa.kassa.name if chiqim.kassa and chiqim.kassa.kassa else 'Noma\'lum',
                'date': chiqim.qachon.strftime('%Y-%m-%d %H:%M'),
            }
            
            data['chiqims'].append(return_pay_data)
            
            total_returned += chiqim.summa
        
        # Qaytarilgan mahsulotlar
        total_returned = 0
        
        for cart in shop.carts:
            for return_item in cart.returns.all():
                return_data = {
                    'product_name': cart.product.name,
                    'quantity': return_item.return_quan,
                    'price': return_item.price,
                    'total': return_item.return_quan * return_item.price,
                    'date': return_item.date.strftime('%Y-%m-%d %H:%M'),
                    'returned_by': return_item.by_user.user.get_full_name() if return_item.by_user else 'Noma\'lum',
                    'status': return_item.get_status_display() if hasattr(return_item, 'get_status_display') else 'Qaytarildi',
                }
                
                data['returns'].append(return_data)
                total_returned += return_data['total']
        
        
        # Jami hisob-kitoblar
        total_summa = sum(item['total'] for item in data['items'])
        total_nds_summa = sum(item['nds_total'] for item in data['items'])
        total_quantity = sum(item['quantity'] for item in data['items'])
        
        # Qoldiq qarz
        remaining_debt = total_summa - total_paid - total_returned
        
        totals = {
            'summa': total_summa,
            'summa_nds': total_nds_summa,
        ...
        messages.error(request, "Sotuv topilmadi")
        return redirect('today_sales')
        ...
        try:
            ...
            for item in return_items:
                ...
                if return_quantity > 0:
                    cart = Cart.objects.get(id=cart_id, shop=shop)
                    
                    # Qaytarilgan miqdor cartdagi miqdordan oshmasligi kerak
                    if return_quantity <= (cart.quantity - cart.total_returned):
                        ...
                        returned_products.append(f"{cart.product.name} - {return_quantity}")

                        # Agar naqd mijoz bo'lsa yoki subtract_from_debt False bo'lsa, chiqim yaratamiz
            
            # Agar subtract_from_debt True bo'lsa va mijoz naqd mijoz bo'lmasa, qarzni kamaytiramiz

            if not subtract_from_debt or (debtor and debtor.fio.lower() == 'naqd'):
                for payment in payments:
                    payment_amount = float(payment.get('amount', 0))
                    valyuta_id = payment.get('valyuta_id')
                    kassa_id = payment.get('kassa_id')
                    
                    if payment_amount > 0 and valyuta_id and kassa_id:
                        try:
                            valyuta = Valyuta.objects.get(id=valyuta_id)
                            kassa = KassaMerge.objects.get(id=kassa_id)
                            
                            # Valyuta kursi bo'yicha konvertatsiya
                            # if shop.valyuta.is_dollar and valyuta.is_som:
                            #     # Dollar -> So'm
                            #     payment_amount_som = payment_amount * shop.kurs
                            # elif shop.valyuta.is_som and valyuta.is_dollar:
                            #     # So'm -> Dollar
                            #     payment_amount_som = payment_amount / shop.kurs
                            # else:
                            #     payment_amount_som = payment_amount
                            
                            # Chiqim yaratish
                            chiqim = Chiqim.objects.create(
                                izox=f"Maxsulot qaytarishi: {cart.product.name}",
                            ...
                            # ReturnProduct ga chiqimni ulash
                            shop.chiqims.add(chiqim)
                            
                            # Kassani yangilash
                            kassa.summa -= payment_amount
                            kassa.save()
                            
                        except (Valyuta.DoesNotExist, KassaMerge.DoesNotExist) as e:
                            continue

                            
            if subtract_from_debt and debtor and debtor.fio.lower() != 'naqd mijoz':
                # Bu yerda qarzni kamaytirish logikasi
                # Sizning qarz tizimingizga qarab implementatsiya
                pass
            
            if returned_products:
                message = f"Quyidagi maxsulotlar qaytarildi:\n" + "\n".join(returned_products)
                if total_return_amount > 0:
                    message += f"\nJami qaytarish summasi: {total_return_amount} {shop.valyuta.name}"
            else:
                message = "Hech qanday maxsulot qaytarilmadi"
...
def get_kassalar_by_valyuta(request):
    valyuta_id = request.GET.get('valyuta_id')
    if valyuta_id:
        kassalar = KassaMerge.objects.filter(
            valyuta_id=valyuta_id, 
            is_active=True,
            summa__gt=0  # Faqat puli bor kassalar
        ).select_related('kassa', 'valyuta')
        
        kassalar_data = []
        for kassa in kassalar:
            kassalar_data.append({
                'id': kassa.id,
                'name': f"{kassa.kassa.name} - {kassa.summa} {kassa.valyuta.name}",
                'balance': kassa.summa
            })
        
        return JsonResponse({'kassalar': kassalar_data})
    
    return JsonResponse({'kassalar': []})
    ...
    if request.method == 'POST':
        try:
            ...
            if kassa != 'qarz':
                ...
                kirim, created = Kirim.objects.get_or_create(payhistory=last, valyuta_id=valyuta, kassa=kassa1)
                # kirim.kassa_id=kassa
                # kirim.valyuta_id=valyuta
                kirim.summa=summa
                kirim.currency=last.currency
                kirim.save()
                
                kassa = KassaMerge.objects.get(id=kassa1.id)
                if not create:
                    kirim.kassa_new = kassa.summa
                kirim.save()
...
def close_cash(request):
    ...
    for kassa in new_kass:
        kassa_id = kassa['id']

        # 🔹 Har bir kassa uchun eng oxirgi yopilish sanasini topamiz
        last_close = (
            CloseCash.objects.filter(kassa__kassa_id=kassa_id)
            .order_by('-date')
            .first()
        )
        ...
        if last_close:
            ...
            # agar hali yopilmagan bo‘lsa, eng boshidan hisoblaymiz
            start_date = None

        # 🔹 Kirimlar
        kirim_qs = Kirim.objects.filter(kassa__kassa_id=kassa_id)
        chiqim_qs = Chiqim.objects.filter(kassa__kassa_id=kassa_id)
        if start_date:
        ...
        kassa_merge = (
            KassaMerge.objects.filter(kassa_id=kassa_id, is_active=True)
            .values('valyuta_id')
            .annotate(summa=Coalesce(Sum(F('summa')), 0, output_field=IntegerField()))
        )

        # 🔹 Lug‘at shaklida tez olish uchun
        kirim_dict = {i['valyuta_id']: i['summa'] for i in kirim}
        chiqim_dict = {i['valyuta_id']: i['summa'] for i in chiqim}
        merge_dict = {i['valyuta_id']: i['summa'] for i in kassa_merge}
        ...
        data.append({
            ...
            'last_close': str(start_date),
            'valyuta': valyuta_list,
        })
...
def confirmed_close_cash_list(request):
    # Filter parametrlarini olish
    start_date = request.GET.get('start_date')
    end_date = request.GET.get('end_date')
    kassa_id = request.GET.get('kassa_id')
    
    # Boshlang'ich so'rov - faqat tasdiqlanganlar
    close = SmenaClose.objects.filter(is_confirmed=True)
    ...
    return render(request, 'confirmed_close_cash_list.html', context)
        ...
        try:
            ...
            for close_cash in smena_close.closecashes.all():
                ...
                try:
                    ...
                    if summa_confirmed:
                        close_cash.summa_confirmed = float(summa_confirmed.replace(',', '').replace(' ', ''))
...
class Recieves(LoginRequiredMixin, TemplateView):
    template_name = 'recieves_new.html'
...
@csrf_exempt
def recieve_create(request):
    try:
        data = json.loads(request.body)
        recieve = Recieve.objects.create(
            name=data.get('name'),
...
@csrf_exempt
def recieve_delete(request, recieve_id):
    try:
        recieve = get_object_or_404(Recieve, id=recieve_id)
        recieve.delete()
...
@require_http_methods(["POST"])
@csrf_exempt
def recieve_item_create(request, recieve_id):
    try:
        data = json.loads(request.body)
        recieve = get_object_or_404(Recieve, id=recieve_id)
        product = get_object_or_404(ProductFilial, id=data.get('product_id'))
        
        recieve_item = RecieveItem.objects.create(
            recieve=recieve,
            product=product,
            quantity=float(data.get('quantity', 0)),
            valyuta=recieve.valyuta
        )
        
        product.quantity += recieve_item.quantity
        product.save()
        
        # Kelish narxlari
        bring_prices = data.get('bring_prices', {})
        for valyuta_id, price in bring_prices.items():
            if price and float(price) > 0:
                valyuta = get_object_or_404(Valyuta, id=valyuta_id)
                ProductBringPrice.objects.create(
                    valyuta=valyuta,
                    product=product,
                    recieveitem=recieve_item,
        ...
        for price_key, price_value in sell_prices.items():
            if price_value and float(price_value) > 0:
                _, pr_price_type_id, valyuta_id = price_key.split('_')
                # price_type = get_object_or_404(PriceType, id=price_type_id)
                # valyuta = get_object_or_404(Valyuta, id=valyuta_id)
                ppt = ProductPriceType.objects.get(
                    id=pr_price_type_id
                )
                ppt.price = float(price_value)
                ppt.save()
        ...
        return JsonResponse({'success': True, 'message': 'Mahsulot qo\'shildi'})
    except Exception as e:
        return JsonResponse({'success': False, 'message': f'Xatolik: {str(e)}'})

@require_http_methods(["PUT"])
@csrf_exempt
def recieve_item_update(request, item_id):
    try:
        data = json.loads(request.body)
        item = get_object_or_404(RecieveItem, id=item_id)
        product = item.product
        
        # Eski miqdorni olib tashlash
        old_quantity = item.quantity
        new_quantity = float(data.get('quantity', 0))
        
        # Mahsulot miqdorini yangilash
        product.quantity = product.quantity - old_quantity + new_quantity
        product.save()
...
@require_http_methods(["DELETE"])
@csrf_exempt
def recieve_item_remove(request, item_id):
    try:
        item = get_object_or_404(RecieveItem, id=item_id)
        product = item.product
        product.quantity -= item.quantity
...
@require_http_methods(["POST"])
@csrf_exempt
def recieve_expense_create(request, recieve_id):
    try:
        data = json.loads(request.body)
        recieve = get_object_or_404(Recieve, id=recieve_id)
        RecieveExpanses.objects.create(
        ...
        return JsonResponse({'success': True, 'message': 'Chiqim qo\'shildi'})
    except Exception as e:
        return JsonResponse({'success': False, 'message': f'Xatolik: {str(e)}'})

@require_http_methods(["PUT"])
@csrf_exempt
def recieve_expense_update(request, expense_id):
    try:
        data = json.loads(request.body)
        expanse = get_object_or_404(RecieveExpanses, id=expense_id)
        expanse.summa = float(data.get('summa', 0))
        expanse.comment = data.get('comment', '')
        expanse.save()
        return JsonResponse({'success': True, 'message': 'Chiqim yangilandi'})
    except Exception as e:
        return JsonResponse({'success': False, 'message': f'Xatolik: {str(e)}'})

@require_http_methods(["DELETE"])
@csrf_exempt
def recieve_expense_delete(request, expense_id):
    try:
        expanse = get_object_or_404(RecieveExpanses, id=expense_id)
        expanse.delete()
        return JsonResponse({'success': True, 'message': 'Chiqim o\'chirildi'})
    except Exception as e:
        return JsonResponse({'success': False, 'message': f'Xatolik: {str(e)}'})

@require_http_methods(["POST"])
@csrf_exempt
def recieve_expense_type_create(request):
    try:
        data = json.loads(request.body)
        expense_type = RecieveExpanseTypes.objects.create(name=data.get('name'))
        return JsonResponse({
            'success': True, 
            'message': 'Chiqim turi qo\'shildi',
            'expense_type': {'id': expense_type.id, 'name': expense_type.name}
        })
    except Exception as e:
        return JsonResponse({'success': False, 'message': f'Xatolik: {str(e)}'})
...
def deliver_create(request):
    try:
        ...
        return JsonResponse({
            ...
            'deliver': {'id': deliver.id, 'name': deliver.name}
        })
    except Exception as e:
        return JsonResponse({'success': False, 'message': f'Xatolik: {str(e)}'})

@require_http_methods(["GET"])
def product_prices_get(request, product_id):
    try:
        product = get_object_or_404(ProductFilial, id=product_id)
        ...
        for valyuta in Valyuta.objects.all():
            ...
            bring_prices.append({
                'valyuta': valyuta.name,
                'valyuta_id': valyuta.id,
                'is_som': valyuta.is_som,
                'is_dollar': valyuta.is_dollar,
                'price': bring_price.price if bring_price else 0
            })
        ...
        for valyuta in Valyuta.objects.all():
            ...
            for price_type in PriceType.objects.all():
                ...
                price_types_data.append({
                    ...
                    'name': ppt.type.name,
                    'price_type_id': ppt.type.id,
                    'summa': ppt.price if ppt else 0
                })
            sell_prices.append({
                'valuta': valyuta.name,
                ...
                'is_som': valyuta.is_som,
                'is_dollar': valyuta.is_dollar,
                'summas': price_types_data
            })
        
        data = {
            'bring_prices': bring_prices,
...
# for i in ReturnProduct.objects.all():
#     if i.chiqim_set:
#         print(i)
