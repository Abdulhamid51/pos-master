...
def daily_data():
    ...
    if date.month == 12:
        ...
        gte = datetime(year, date.month, date.day, 0, 0, 0)
        lte = datetime(year, date.month, date.day, 0, 0, 0) + timedelta(days=1)
        print(lte)
...
def DeliverHistory(request):
    ...
    for d in debts:
        dsom += d.som
        ddollar += d.dollar
    print(dsom, 'bbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbb')
...
def add_tolov(request):
    ...
    # for d in DebtDeliver.objects.filter(deliver_id=deliver_id):
    #     dsom += d.som
    #     ddollar += d.dollar
...
# for deliver in Deliver.objects.all():
#     qwert = ProductFilial.objects.filter(deliver=deliver)
#     print(qwert)
#     # print(deliver.products1.all()) 
...
class Delivers(LoginRequiredMixin, TemplateView):
    ...
    def get_context_data(self, **kwargs):
        ...
        for dl in Deliver.objects.all():
            ...
            print(dl.name, dsom, 'aaaaaaaaaaaaaaaaaaaaaaaaaaaaaa')
...
def kassa(request):
    ...
    hodimlar_qarz = []
    shu_oylik_chiqimlar = Chiqim.objects.filter(qachon__year=bugun.year, qachon__month=bugun.month).order_by('-qachon')
    shu_oylik_kirimlar = Kirim.objects.filter(qachon__year=bugun.year, qachon__month=bugun.month).order_by('-qachon')
...
def kirim_qilish(request):
    ...
    if request.method == 'POST':
        ...
        chiqim = Kirim.objects.create(izox=izox)
        if qancha_som:
            chiqim.qancha_som = qancha_som
            kassa_var.som += int(qancha_som)
            
        if plastik:
            chiqim.plastik = qancha_som
