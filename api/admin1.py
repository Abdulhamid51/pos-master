...
admin.site.register(DeliverPaymentsAll)
...
class ProductFilialAdmin(ExportActionMixin, admin.ModelAdmin):
    list_display = ('id', 'som', 'dollar', 'quantity', 'filial', 'barcode', 'get_delivers')
    search_fields = ('id', 'som', 'dollar', 'quantity', 'filial__name', 'barcode')
    list_filter = ('filial',)
    
    def get_delivers(self, obj):
        return ",".join([str(p.name) for p in obj.deliver.all()])
...
class RecieveItemAdmin(admin.ModelAdmin):
    ...
    search_fields = ('id', 'som', 'dollar', 'kurs', 'quantity')
